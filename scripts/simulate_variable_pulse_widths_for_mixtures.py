...
def simulate_variable_pulse_widths_for_mixtures():
    ...
    out_dir = "variable_pulse_width_simulations_for_mixtures"

    # Pulse widths to simulate
    pulse_width_values = [0.0005, 0.002, 0.008, 0.016]
    ...
    replicates_per_parameter_set = 2
    ...
    def make_out_paths(pulse_width, replicate):
        movie_path = os.path.join(
            out_dir,
            f"mixture_pulse-width-{pulse_width}_rep-{replicate}.tif"
        )
    ...
    n_simulations = len(pulse_width_values) * replicates_per_parameter_set
    c = 0
    ...
    for pulse_width in pulse_width_values:
        for replicate in range(replicates_per_parameter_set):
            print(f"Simulation {c+1}/{n_simulations}: " \
                f"pulse_width = {pulse_width}, replicate = {replicate}")
            movie_path, csv_path = make_out_paths(pulse_width, replicate)
            ...
            with SPTSimulator(
                ...
                TM = np.identity(len(diff_coefs)) * occs,
            ) as simulator:
...
if __name__ == "__main__":
    simulate_variable_pulse_widths_for_mixtures()
