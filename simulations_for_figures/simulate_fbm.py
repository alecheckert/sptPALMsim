...
def simulate_fbm():
    ...
    We also make a version of each simulation with and without motion
    blur, which turns out to have important effects on Hurst parameter
    estimation. """
    # Relative path to output directory
    out_dir = "fbm_simulations"
    ...
    def make_out_paths(model_index: int, replicate: int, motion_blur: bool) -> (str, str):
        if motion_blur:
            movie_path = os.path.join(
                out_dir,
                f"fbm_model-index-{model_index}_rep-{replicate}_motion-blur.tif",
            )
        else:
            movie_path = os.path.join(
                out_dir,
                f"fbm_model-index-{model_index}_rep-{replicate}_no-motion-blur.tif",
            )
    ...
    for model_index, (diff_coefs, hurst_pars, occs) in enumerate(models):
        ...
        for replicate in range(simulations_per_model):
            ...
            # Simulate without motion blur (we use temporal_bin_rate = 1
            # to simulate 1 position of the particle per frame interval)
            movie_path, csv_path = make_out_paths(model_index, replicate, motion_blur=False)
            ...
            # Simulate with motion blur (we use temporal_bin_rate = 60
            # to simulate 60 positions of the particle per frame interval)
            movie_path, csv_path = make_out_paths(model_index, replicate, motion_blur=True)
            if os.path.isfile(movie_path):
                print(f"  output {movie_path} already exists; moving on")
                c += 1
                continue
